...
//! stream bridge, a stub capability hook, and a mock edge registration
//! server that answers framed RPC messages.
...
use libcfd_rpc::{rpc_capnp, tunnelrpc_capnp};
...
pub async fn serve_mock<S: libcfd_rpc::AsyncStream + Unpin>(mut stream: S) -> Vec<Received> {
...
/// Same as [`serve_mock`] but also stops after the release message so tests
/// of the close path can observe it.
...
/// Like [`serve_mock`] but answers every call with an EDUPCONN error.
pub async fn serve_mock_err<S: libcfd_rpc::AsyncStream + Unpin>(mut stream: S) -> Vec<Received> {
    let mut received = Vec::new();
    loop {
        let reader = match read_message_or_eof(&mut stream).await {
            Some(r) => r,
